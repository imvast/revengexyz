...
(function($, window, document, undefined) {
    ...
    Plugin.prototype = {
        init: function() {
            ...
            if (version)
                androidCompatible = parseFloat(version[1]) > 3;
        ...
        setup: function() {
            ...
            if (isFunction(this.options.ready))
                this.options.ready();
        },
        _bindEvents: function() {
            ...
            this.node.on('scrollToStage', function(event, floor) {
                if (typeof floor == 'string') {
                    ...
                    if (floorId !== -1)
                        self.scrollToFloor(floorId);
                } else if (typeof floor == 'number') {
                    if (floor > self.nodeChildren.length)
                        return;
            ...
            if (this.options.wheelNavigation) {
                this.node.on('mousewheel.pofloor', function(e) {
                    setTimeout(function() {
                        if (!self.scrollInChildren)
                            self._handleMouseWheelEvent(e);
                    }, 10);
                });
            ...
            if (this.options.swipeNavigation) {
                ...
                if (this.options.swipeNavigation !== 'mobile-only')
                    touchEvent += ' mousedown.pofloor mouseup.pofloor';
        ...
        _handleMouseWheelEvent: function(event) {
            ...
            if (Math.abs(mouseX) > Math.abs(mouseY) && mouseX > 0)
                this.scrollToDirection('left');
            if (Math.abs(mouseX) > Math.abs(mouseY) && mouseX < 0)
                this.scrollToDirection('right');
            if (Math.abs(mouseY) > Math.abs(mouseX) && mouseY > 0)
                this.scrollToDirection('up');
            if (Math.abs(mouseY) > Math.abs(mouseX) && mouseY < 0)
                this.scrollToDirection('down');
        },
        _handleTouchEvent: function(event) {
            ...
            case 'mouseup':
                ...
                if (velocityX > this.options.swipeVelocity && Math.abs(distanceX) > Math.abs(distanceY) && distanceX < 0)
                    this.scrollToDirection('left');
                if (velocityX > this.options.swipeVelocity && Math.abs(distanceX) > Math.abs(distanceY) && distanceX > 0)
                    this.scrollToDirection('right');
                if (velocityY > this.options.swipeVelocity && Math.abs(distanceX) < Math.abs(distanceY) && distanceY < 0)
                    this.scrollToDirection('up');
                if (velocityY > this.options.swipeVelocity && Math.abs(distanceX) < Math.abs(distanceY) && distanceY > 0)
                    this.scrollToDirection('down');
                break;
        ...
        _positionElement: function() {
            var self = this;
            if (this.directionIsArray)
                this._generateFloorMap();
            this.node.css({
                'position': 'absolute',
                'overflow': 'hidden',
                'top': '0',
                    ...
                    $(this).css({
                        'transform': function() {
                            ...
                            if (self.directionIsArray)
                                return 'translateY(' + self.options.direction[index][self.AXIS_Y] * 100 + '%) translateX(' + self.options.direction[index][self.AXIS_X] * 100 + '%)';
                        }
                    });
                } else {
                    $(this).css({
                        'top': function() {
                            if (self.options.direction === 'y')
                                return index * 100 + '%';
                            if (self.directionIsArray)
                                return self.options.direction[index][self.AXIS_Y] * 100 + '%';
                        },
                        'left': function() {
                            if (self.options.direction === 'x')
                                return index * 100 + '%';
                            if (self.directionIsArray)
                                return self.options.direction[index][self.AXIS_X] * 100 + '%';
                        },
                    });
                }
            });
        ...
        _emitConsoleMessage: function(type, warn) {
            if (type == "error")
                console.error("pofloor.js: " + warn);
            if (type == "warn")
                console.warn("pofloor.js: " + warn);
        },
        ...
        scrollToFloor: function(floor) {
            if (isString(floor) && existInArray(this.options.pofloorFloorName, floor))
                floor = this.options.pofloorFloorName.indexOf(floor);
            var self = this;
            var animate = (floor === this.floorActive) ? false : true;
            if (this.NW !== this.node.width())
                this.NW = this.node.width();
            if (this.NH !== this.node.height())
                this.NH = this.node.height();
        ...
        scrollToDirection: function(direction) {
            ...
            if ((self.options.direction == 'y' && directionIsHorizontal) || (self.options.direction == 'x' && directionIsVertical))
                return;
            if ((self.options.direction == 'y' && direction == 'down') || (self.options.direction == 'x' && direction == 'right'))
                return self.next();
            if ((self.options.direction == 'y' && direction == 'up') || (self.options.direction == 'x' && direction == 'left'))
                return self.prev();
            if (self.directionIsArray) {
                var floorObject = self.floorMap[self.floorActive];
                var directFloor = floorObject[direction];
                if (isNumber(directFloor))
                    return self.scrollToFloor(directFloor);
                var closestFloor = floorObject.closest[direction];
                if (isTrue(self.options.jump) && isNumber(closestFloor))
                    return self.scrollToFloor(closestFloor);
                ...
                if ((self.options.loop == 'increment-x' && directionIsHorizontal) || self.options.loop == 'increment') {
                    if (self.floorActive == self.floorMap.furthest_x)
                        return self.scrollToFloor(self.floorMap.closest_x);
                    if (self.floorActive == self.floorMap.closest_x)
                        return self.scrollToFloor(self.floorMap.furthest_x);
                }
                if ((self.options.loop == 'increment-y' && directionIsVertical) || self.options.loop == 'increment') {
                    if (self.floorActive == self.floorMap.furthest_y)
                        return self.scrollToFloor(self.floorMap.closest_y);
                    if (self.floorActive == self.floorMap.closest_y)
                        return self.scrollToFloor(self.floorMap.furthest_y);
                }
            }
        },
        _getDirectFloorIndex: function(DA, floorIndex, direction) {
            ...
            if (direction == 'right')
                floorTarget[this.AXIS_X] += 1;
            if (direction == 'left')
                floorTarget[this.AXIS_X] -= 1;
            if (direction == 'up')
                floorTarget[this.AXIS_Y] -= 1;
            if (direction == 'down')
                floorTarget[this.AXIS_Y] += 1;
...
;(function($, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        writeDigitsToDom: function(w, v, c) {
            var t = this, s = t.settings, wd = s.wrapDigitsTag, j = $(t.element), we = j.find(w), wte, v = v.toString(), vs = [], diff;
    ...
    f.pobubble = function(l, t) {
        ...
        var w = function() {
          ...
          , s = function(e) {
            ...
            a.options.horizontal && (d.left = c[0],
            b.x = c[0]);
            a.options.vertical && (d.top = c[1],
            b.y = c[1]);
